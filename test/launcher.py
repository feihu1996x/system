...
'''
python manager.py runTest -m Test  (  test/cases/Test.py )
python manager.py runTest -m test/Index (  test/cases/test/Index.py )
...
class runTest( Command ):
    ...
    def run(self,*args,**kwargs):
        ...
        parser.add_argument("-m","--name",dest = "name",metavar = "name", help="指定测试用例名称",required=True)
        parser.add_argument("-a","--act",dest = "act",metavar = "act", help="测试动作",required=False)
        ...
        try:
            import_string = "from test.cases.%s import TestCase as  test_target" % ( module_name )
